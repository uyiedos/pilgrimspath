-- =================================================================
-- READING PLAN STAGE COMPLETION LOGGING - VERSION 2
-- =================================================================
-- Run this in your Supabase SQL Editor to add proper stage completion tracking
-- This version fixes SQL errors and adds unique stage IDs

-- 1. CREATE TABLE FOR INDIVIDUAL STAGE COMPLETIONS WITH STAGE IDs
CREATE TABLE IF NOT EXISTS public.plan_stage_completions (
    id uuid DEFAULT uuid_generate_v4() PRIMARY KEY,
    user_id uuid REFERENCES public.users(id) NOT NULL,
    plan_id text NOT NULL,
    stage_id text NOT NULL, -- Unique identifier for each stage (e.g., "john_3_16")
...
CREATE INDEX IF NOT EXISTS idx_plan_stage_completions_user_stage ON public.plan_stage_completions(user_id, stage_id);
...
-- 6. CREATE FUNCTION TO START A STAGE (LOGS START TIME)
CREATE OR REPLACE FUNCTION start_plan_stage(
    p_user_id uuid,
    p_plan_id text,
    p_stage_id text,
    p_stage_number int,
...
BEGIN
    ...
    INSERT INTO public.plan_stage_completions (
        user_id, plan_id, stage_id, stage_number, stage_title, reading_reference, started_at
    ) VALUES (
        p_user_id, p_plan_id, p_stage_id, p_stage_number, p_stage_title, p_reading_reference, now()
    )
...
-- 7. CREATE FUNCTION TO COMPLETE A STAGE WITH XP AWARD
CREATE OR REPLACE FUNCTION complete_plan_stage(
    p_user_id uuid,
    p_plan_id text,
    p_stage_id text,
    p_stage_number int,
...
-- 8. CREATE FUNCTION TO GET USER'S STAGE COMPLETION STATUS
...
BEGIN
    RETURN QUERY
    SELECT 
        -- Generate stage_id from reading reference (e.g., "john_3_16")
        regexp_replace(lower(regexp_replace(p_reading_reference, '[^a-zA-Z0-9]', '_', 'g')), '_+', '_', 'g') as stage_id,
    ...
    LEFT JOIN public.plan_stage_completions c ON 
        ...
        c.stage_id = regexp_replace(lower(regexp_replace(d->>'reading', '[^a-zA-Z0-9]', '_', 'g')), '_+', '_', 'g')
    ORDER BY d.stage::int;
END;
$$;

-- 9. FIX THE VIEW (CORRECTED COLUMN REFERENCES)
