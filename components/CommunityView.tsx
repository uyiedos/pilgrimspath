...
const CommunityView: React.FC<CommunityViewProps> = ({ user, onBack, language, onAddPoints, onUnlockAchievement, onConvertGuest, spendPoints, initialCommunityId }) => {
  ...
  const handleJoin = async (community: Community) => {
    ...
    // Unlock Achievement
    if (onUnlockAchievement) onUnlockAchievement('fellowship_found');
    AudioSystem.playAchievement();
        ...
        <div className="relative z-10 w-full max-w-6xl">
            {selectedCommunity ? (
                ...
                <div className="space-y-8 animate-fade-in">
                    <div className="bg-black/40 backdrop-blur-2xl p-6 md:p-10 rounded-[3rem] border-4 border-white/10 shadow-2xl flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
                        ...
                        <div className="flex flex-wrap justify-center gap-2 bg-white/5 p-2 rounded-[1.5rem] border border-white/10 backdrop-blur-md relative z-10 w-full md:w-auto">
                            <button onClick={() => setActiveTab('find')} className={`flex-1 md:flex-none px-4 md:px-6 py-2 md:py-3 rounded-xl font-retro text-[9px] uppercase transition-all whitespace-nowrap ${activeTab === 'find' ? 'bg-yellow-600 text-white shadow-xl border-2 border-yellow-400 scale-105' : 'text-gray-500 hover:text-gray-300'}`}>Find Ministries</button>
                            <button onClick={() => setActiveTab('my')} className={`flex-1 md:flex-none px-4 md:px-6 py-2 md:py-3 rounded-xl font-retro text-[9px] uppercase transition-all whitespace-nowrap ${activeTab === 'my' ? 'bg-yellow-600 text-white shadow-xl border-2 border-yellow-400 scale-105' : 'text-gray-500 hover:text-gray-300'}`}>My Covenant</button>
                            <Button onClick={onBack} variant="secondary" className="px-4 py-2 rounded-xl text-[9px]">🏠 Home</Button>
                        </div>
                    </div>
