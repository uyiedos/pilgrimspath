...
const AuthView: React.FC<AuthViewProps> = ({ onLogin }) => {
  ...
  useEffect(() => {
    ...
    const fetchActivity = async () => {
        try {
            const { data } = await supabase
                .from('activity_feed')
                .select('username, activity_type, details, created_at')
            ...
            if (data && data.length > 0) {
                const activities = data.map((item: any) => {
                    ...
                    const title = details.title || (item.activity_type === 'join' ? 'New Pilgrim' : 'Event');
                    const icon = details.icon || '✨';
                    const user = item.username || 'Anonymous';
                    const reward = details.reward ? `+${details.reward} XP` : '';
                    return `${icon} ${user}: ${title} ${reward}`;
                });
                setRecentActivity(activities);
            } else {
                // Only show this default if no real data is found.
                // We ensure no "Shield of Faith" mock data is here.
                setRecentActivity(["✨ The Journey Network is Online"]);
            }
        } catch (e) {
    ...
    fetchActivity();
    const interval = setInterval(fetchActivity, 10000); // Poll every 10s for new actions
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        // If referral code provided, validate and set referred_by
        if (referralCode.trim()) {
          const { data: referrer } = await supabase.from('users').select('id').eq('referral_code', referralCode.trim()).single();
          if (referrer) {
            userPayload.referred_by = referrer.id;
          } else {
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 md:py-12 flex flex-col items-center">
           ...
           <form onSubmit={handleAuth} className="space-y-5">
              {mode === 'signup' && (
                  <>
                      ...
                      <div>
                          <input type="text" placeholder="Referral Code (Optional)" className="w-full bg-gray-900/80 border-2 border-gray-700 rounded-xl p-4 text-white focus:border-yellow-500 outline-none transition-all placeholder-gray-600" value={referralCode} onChange={e => setReferralCode(e.target.value.toUpperCase())} />
                      </div>
                  </>
              )}
              <div>
        ...
        <div className="mt-20 max-w-6xl w-full">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                {/* Removed Forge, Market, Vault as requested */}
            </div>
        ...
        <div className="mt-16 w-full max-w-3xl bg-gray-900/80 backdrop-blur-sm rounded-full border border-white/10 p-3 flex items-center justify-center overflow-hidden shadow-2xl">
            <div className="flex gap-4 md:gap-8 items-center animate-slide-in opacity-90 overflow-x-hidden">
                <span className="text-[10px] uppercase font-mono text-green-400 flex items-center gap-2 shrink-0">
                    <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
                    Live Network
                </span>
                <div className="flex gap-8 overflow-hidden whitespace-nowrap mask-linear">
                    {recentActivity.map((act, i) => (
                        <span key={i} className="text-[10px] text-white font-mono">{act}</span>
                    ))}
                </div>
            </div>
        </div>
