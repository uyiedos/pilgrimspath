...
const BrowserView: React.FC<BrowserViewProps> = ({ onBack, onAddPoints, onUnlockAchievement }) => {
  ...
  const handleSearch = async (overrideQuery?: string) => {
    ...
    try {
      ...
      if (data && (data.resources?.length > 0 || data.summary)) {
        setResults(data.resources || []);
        setSummary(data.summary || "Here are the resources revealed for your journey.");
        
        onAddPoints(5); 
        if (onUnlockAchievement) onUnlockAchievement('digital_scribe');
            ...
            <div className="max-w-4xl mx-auto flex flex-col items-center min-h-min pb-20">
                ...
                <div className={`w-full transition-all duration-500 ${hasSearched ? 'translate-y-0' : 'translate-y-[10vh] md:translate-y-[20vh]'}`}>
                    ...
                    <div className="text-right mt-2 mr-2">
                        <span className="text-[9px] font-mono text-gray-500 uppercase tracking-widest">Powered by Groq AI</span>
                    </div>
                    ...
                    <div className="w-full mt-8 md:mt-12 animate-slide-up space-y-8">
                        ...
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-20 gap-4">
                                ...
                                <p className="text-yellow-500 font-retro text-xs animate-pulse">Consulting the Groq Archives...</p>
                            </div>
                        ) : (
                            <>
                                {summary && (
