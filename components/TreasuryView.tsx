...
const TreasuryView: React.FC<TreasuryViewProps> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-[#050508] p-2 pt-16 md:p-8 flex flex-col items-center animate-fade-in custom-scroll relative">
      ...
      <div className="relative z-10 w-full max-w-7xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-8 bg-gray-900/60 backdrop-blur-3xl p-6 rounded-[2rem] border border-white/5 shadow-2xl">
            ...
            <div className="flex gap-2 bg-black/40 p-1.5 rounded-xl border border-white/10">
                {(['dashboard', 'ledger', 'holders'] as const).map(tab => (
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-lg text-[10px] font-retro uppercase transition-all ${activeTab === tab ? 'bg-yellow-600 text-black shadow-lg' : 'text-gray-500 hover:text-white'}`}
                    >
                        {tab}
                    </button>
                ))}
            ...
            <Button onClick={onBack} variant="secondary" className="hidden md:block">EXIT</Button>
        </div>
