...
import { forgePlan } from '../services/geminiService';
...
const buildUuid = () => {
  ...
  // RFC4122 v4 UUID fallback (valid for Postgres uuid column)
...
const PlansView: React.FC<PlansViewProps> = ({ user, onBack, onAddPoints, language, plans, onUpdatePlans, initialPlanId, spendPoints, onUnlockAchievement, onAwardBadge }) => {
  ...
  // Load user plans from Supabase on mount
  useEffect(() => {
    const loadUserPlans = async () => {
      ...
      if (userPlans && !error) {
        ...
        const mergedPlans = plans.map(plan => {
          const userPlan = userPlans.find(up => up.title === plan.title);
          if (userPlan) {
            return {
              ...plan,
              isActive: userPlan.is_active,
              progress: userPlan.progress || 0,
              startDate: userPlan.start_date,
              lastCompletedDate: userPlan.last_completed_date,
              days: userPlan.days_json || plan.days
            };
          }
          return plan;
        });
  ...
  const fetchScripture = async (reference: string) => {
    ...
    try {
      // Skip external fetch for non-verse labels like "Day 3 Reading"
      if (/\b(Day|Stage)\b/i.test(reference) || /Reading/i.test(reference)) {
        setScriptureText("Open today's reading inside the app. Scripture fetch is only used for direct verse references (e.g., 'John 3:16').");
        return;
      }
  ...
  const savePlanProgress = async (plan: BiblePlan) => {
      ...
      if (plan.days && plan.days.length > 0) {
          basePayload.days_json = plan.days;
      }

      // The DB doesn't have a compatible unique constraint for ON CONFLICT,
      // so we do a safe select → update/insert flow.
      ...
      if (selectErr) {
        console.error("Failed to check existing plan row:", selectErr);
        return;
      }

      if (existing?.id) {
        const { user_id: _skipUserId, title: _skipTitle, ...updatePayload } = basePayload;
        ...
        if (updateErr) {
          console.error("Failed to update plan:", updateErr);
        }
        return;
      }

      const insertPayload = { id: buildUuid(), ...basePayload };
      const { error: insertErr } = await supabase.from('user_plans').insert([insertPayload]);
      if (insertErr) {
  ...
  const recordPlanCompletion = async () => {
    if (!user || !selectedPlan || !currentStage) return;
    const nowIso = new Date().toISOString();
    const startedAt = stageStartedAt || nowIso;

    const stageTitle = currentStage.topic
      ? `${selectedPlan.title} • ${currentStage.topic}`
      : `${selectedPlan.title} • Stage ${currentStage.day}`;

    const payload: any = {
      id: buildUuid(),
      user_id: user.id,
      stage_title: stageTitle,
      reading_reference: currentStage.reading,
      user_reflection: quizAnswer,
      started_at: startedAt,
      completed_at: nowIso
    };

    const { error } = await supabase.from('plan_stage_completions').insert([payload]);
    if (error) {
      console.warn('Failed to log stage completion into plan_stage_completions:', error);
      return false;
    }
    return true;
  };
  ...
  const handleForgePath = async () => {
      ...
      try {
          const days = await forgePlan(forgeTitle, forgeFocus, forgeDuration);
