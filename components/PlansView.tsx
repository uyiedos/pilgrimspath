...
const PlansView: React.FC<PlansViewProps> = ({ user, onBack, onAddPoints, language, plans, onUpdatePlans, initialPlanId, spendPoints, onUnlockAchievement, onAwardBadge }) => {
  ...
  useEffect(() => {
    if (selectedPlanId && user) {
      loadCompletedStages(selectedPlanId);
    }
  }, [selectedPlanId, user]);
  ...
  // Discover now shows ALL plans as a catalog, regardless of active status
  ...
  // Helper function to generate stage ID from reading reference
  const generateStageId = (readingReference: string): string => {
    return readingReference.toLowerCase()
      .replace(/[^a-z0-9]/g, '_')
      .replace(/_+/g, '_')
      .replace(/^_|_$/g, '');
  };

  const startStage = async (planId: string, stage: any) => {
    if (!user || user.id.startsWith('offline-')) return;
    
    const stageId = generateStageId(stage.reading);
    
    try {
      const { data, error } = await supabase.rpc('start_plan_stage', {
        p_user_id: user.id,
        p_plan_id: planId,
        p_stage_identifier: stageId, // Updated parameter name
        p_stage_number: stage.day,
        p_stage_title: stage.topic || `Stage ${stage.day}`,
        p_reading_reference: stage.reading
      });
  ...
  const completeStage = async (planId: string, stage: any, reflection: string) => {
    if (!user || user.id.startsWith('offline-')) return false;
    
    const stageId = generateStageId(stage.reading);
    
    try {
      const { data, error } = await supabase.rpc('complete_plan_stage', {
        p_user_id: user.id,
        p_plan_id: planId,
        p_stage_identifier: stageId, // Updated parameter name
        p_stage_number: stage.day,
        p_stage_title: stage.topic || `Stage ${stage.day}`,
        p_reading_reference: stage.reading,
        p_user_reflection: reflection,
        p_xp_awarded: 50
      });
      
      if (error) {
        console.error("Failed to complete stage:", error);
        // If database logging fails, still allow local completion
        return true;
      }
  ...
  const loadCompletedStages = async (planId: string) => {
    if (!user || user.id.startsWith('offline-')) return;
    
    try {
      const { data, error } = await supabase.rpc('get_user_plan_stage_status', {
        p_user_id: user.id,
        p_plan_id: planId
      });
      ...
      if (error) {
        ...
        // Initialize empty set to prevent UI issues
      ...
      if (data) {
        ...
        data.forEach((stage: any) => {
          if (stage.is_completed) {
            completed.add(stage.stage_identifier); // Updated field name
          }
        });
        setCompletedStages(completed);
        console.log("Loaded completed stages:", completed);
      }
    } catch (err) {
      console.error("Error loading completed stages:", err);
      // Initialize empty set to prevent UI issues
      setCompletedStages(new Set());
    }
  };
  ...
  const savePlanProgress = async (plan: BiblePlan) => {
      ...
      const payload: any = {
          ...
          user_id: user.id,
          title: plan.title,
          description: plan.desc, // Map desc to description
      ...
      // Only sync days_json if it exists (crucial for custom forged plans)
      if (plan.days && plan.days.length > 0) {
          payload.days_json = plan.days;
      }

      // Upsert without explicit onConflict relies on the Primary Key (user_id, id)
      const { error } = await supabase.from('user_plans').upsert(payload);
  ...
  const startReading = (stage: any) => {
    ...
    // Log stage start when user begins reading
    if (selectedPlan) {
      startStage(selectedPlan.id, stage);
    }
  };

  const handleCompleteObjective = async () => {
    if (!quizAnswer.trim() || !selectedPlan || !currentStage) return;
    setIsSubmitting(true);
    
    // Complete the stage using the new function
    const isNewCompletion = await completeStage(selectedPlan.id, currentStage, quizAnswer.trim());
    
    // Refresh completed stages after completion
    if (isNewCompletion) {
      await loadCompletedStages(selectedPlan.id);
    }
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0f] p-4 pt-20 md:p-8 flex flex-col items-center bg-[url('https://www.transparenttextures.com/patterns/dark-matter.png')] animate-fade-in custom-scroll relative">
      
      {/* READING SESSION OVERLAY - MODAL STYLE FOR SCROLLING */}
      {sessionStage !== 'IDLE' && (
      ...
      <div className="relative w-full max-w-6xl">
        ...
        {selectedPlan ? (
          ...
          <div className="animate-slide-up space-y-6 max-w-3xl mx-auto pb-24">
                   ...
                   <div className="flex-1 space-y-4">
                      ...
                      <div className="flex flex-wrap gap-4 justify-center md:justify-start">
                        <div className="bg-white/5 px-4 py-2 rounded-xl border border-white/10">
                           <span className="text-[8px] text-gray-500 block uppercase mb-1">Length</span>
                           <span className="text-white font-bold">{selectedPlan.duration} Stages</span>
                        </div>
                        <div className="bg-white/5 px-4 py-2 rounded-xl border border-white/10">
                           <span className="text-[8px] text-gray-500 block uppercase mb-1">Final Reward</span>
             ...
             <div className="space-y-4">
                ...
                {selectedPlan.days?.map((day) => {
                   const stageId = generateStageId(day.reading);
                   const isCompleted = completedStages.has(stageId);
                   // Find the highest completed stage number to determine next
                   const completedStageNumbers = Array.from(completedStages).map(id => {
                     const stage = selectedPlan.days?.find(d => generateStageId(d.reading) === id);
                     return stage?.day || 0;
                   });
                   const maxCompletedStage = Math.max(0, ...completedStageNumbers);
                   const isNext = day.day === maxCompletedStage + 1 && !isCompleted;
          ...
          <div className="animate-fade-in space-y-8">
            <div className="flex justify-center gap-3 bg-black/40 p-2 rounded-[2.5rem] border border-white/10 backdrop-blur-md max-w-xl mx-auto shadow-2xl">
               <button 
