...
const GlobalChat: React.FC<GlobalChatProps> = ({ user, onAddPoints, onUnlockAchievement }) => {
  ...
  return (
    <>
      {/* FLOATING TOGGLE BUTTON - SHIFTED UP to clear footers */}
      <button 
        onClick={() => { setShowChat(!showChat); setUnreadCount(0); AudioSystem.playVoxelTap(); }} 
        className="fixed bottom-20 right-6 z-[100] w-16 h-16 bg-blue-600 hover:bg-blue-500 rounded-full shadow-[0_0_30px_rgba(37,99,235,0.4)] flex items-center justify-center transition-all hover:scale-110 active:scale-95 border-4 border-white group"
