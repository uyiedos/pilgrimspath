...
import { findBiblicalVerse, generateImagePrompt } from '../services/geminiService';
...
const ForgeView: React.FC<ForgeViewProps> = ({ user, totalPoints, onBack, onUpdateUser, spendPoints, onUnlockAchievement, collectedVerses = [], language, initialArtifactId, onAddPoints, onSync }) => {
  ...
  const handleForge = async () => {
    ...
    try {
      // 1. Generate optimized prompt via Groq
      let basePrompt = "";
      if (activeTab === 'identity') {
        basePrompt = `Biblical character portrait for ${user.archetype || 'Seeker'}, ${protocol.adj}, divine aura.`;
      } else if (activeTab === 'sanctuary') {
        basePrompt = `Heavenly sanctuary landscape, ${protocol.adj}, peaceful atmosphere.`;
      } else if (activeTab === 'scripture' && foundVerse) {
        basePrompt = `Sacred artifact diorama representing: ${foundVerse.text}, ${protocol.adj}.`;
      }

      const finalPrompt = await generateImagePrompt(basePrompt, aesthetic);
      
      setForgeStatus("Manifesting Visuals...");
      ...
      const { data: rpcData, error: rpcError } = await supabase.rpc('forge_and_list_artifact', {
          ...
          p_style_prompt: activeTab === 'scripture' ? `Verse: ${foundVerse?.reference}` : finalPrompt,
      ...
      if (onSync) onSync(); 
      else onUpdateUser({ ...user, totalPoints: (user.totalPoints || 0) - protocol.cost });
  ...
  const handleBurn = async (artifact: ForgedArtifact) => {
    ...
    try {
        ...
        if (onSync) onSync();
        else onUpdateUser({ ...user, totalPoints: user.totalPoints + data.reclaimed_xp });
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center pt-20 md:pt-24 p-4 relative z-10 custom-scroll">
      {viewingArtifact && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 p-4 animate-fade-in backdrop-blur-md">
           <div className="relative bg-gray-900 border-4 border-yellow-600 rounded-3xl w-full max-w-4xl max-h-[85vh] flex flex-col md:flex-row overflow-hidden shadow-2xl">
              ...
              {/* Image Section */}
              <div className="w-full md:w-1/2 bg-black relative shrink-0 h-64 md:h-auto border-b-4 md:border-b-0 md:border-r-4 border-yellow-600">
                 {user && (user.avatar === viewingArtifact.avatar_url || user.sanctuaryBackground === viewingArtifact.avatar_url) ? (
                    <div className="w-full h-full flex flex-col items-center justify-center bg-gray-900">
                        <span className="text-6xl animate-pulse">🛡️</span>
                        <span className="text-yellow-500 font-retro text-sm mt-4 tracking-widest uppercase border-2 border-yellow-500 px-4 py-1 rounded-full">Active Equipment</span>
                    </div>
                 ) : (
                    <LazyImage src={viewingArtifact.avatar_url} className="w-full h-full" alt="Artifact" priority />
                 )}
              </div>

              {/* Details Section */}
              <div className="flex flex-col flex-1 p-6 md:p-8 overflow-y-auto custom-scroll">
                 <div className="flex-1">
                    <div className="flex justify-between items-start mb-2 pr-8">
                        <h3 className="text-xl md:text-2xl font-retro text-yellow-500 uppercase leading-tight">{viewingArtifact.collection_name}</h3>
                    </div>
                    ...
                    <div className="bg-black/40 p-4 rounded-xl border border-white/10 mb-6">
                       ...
                       <p className="text-gray-300 italic font-serif text-sm line-clamp-4">"{viewingArtifact.style_prompt}"</p>
                    </div>
                 ...
                 <div className="space-y-3 mt-auto">
                    <div className="flex gap-2">
                        <button onClick={() => handleShareArtifact(viewingArtifact)} className="flex-1 py-3 rounded-xl bg-blue-600/20 text-blue-400 border border-blue-500/50 hover:bg-blue-600 hover:text-white transition-all text-xs font-bold uppercase">Share Artifact</button>
                    </div>
                    {user && user.id === viewingArtifact.user_id && (
                        <>
                            {!(user.avatar === viewingArtifact.avatar_url || user.sanctuaryBackground === viewingArtifact.avatar_url) ? (
                                <Button onClick={() => handleEquip(viewingArtifact)} disabled={isEquipping} className="w-full bg-green-600 border-green-400 py-3 text-xs">{isEquipping ? 'EQUIPPING...' : 'EQUIP ARTIFACT'}</Button>
                            ) : (
                            ...
                            <button onClick={() => handleBurn(viewingArtifact)} disabled={isBurning || user.avatar === viewingArtifact.avatar_url || user.sanctuaryBackground === viewingArtifact.avatar_url} className="w-full py-3 rounded-xl border-2 border-red-900 bg-red-950/30 text-red-500 font-retro text-[10px] hover:bg-red-600 hover:text-white transition-all uppercase tracking-widest disabled:opacity-30">
                                {isBurning ? 'DISSOLVING...' : `DISSOLVE (+${Math.floor((viewingArtifact.attached_xp || 0) * 0.5)} XP)`}
                            </button>
                        </>
                    )}
                 </div>
              ...
              {isForging ? (
                ...
                <>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm p-6 text-center">
                        ...
                        <p className="text-gray-400 text-xs max-w-xs">Your artifact has been forged and automatically listed. Upon sale, you will receive your investment + 20% profit.</p>
                    </div>
                </>
              ) : (
                ...
                {myCollection.length === 0 ? (
                    ...
                    myCollection.map((item) => {
                        ...
                        <div key={item.id} onClick={() => setViewingArtifact(item)} className={`aspect-square bg-gray-900 rounded-lg md:rounded-xl overflow-hidden border md:border-2 transition-all cursor-pointer relative group ${isEquipped ? 'border-yellow-500 shadow-lg scale-105' : 'border-gray-800 hover:border-yellow-500 hover:scale-105'}`}>
                            {isEquipped ? (
                                <div className="w-full h-full flex flex-col items-center justify-center bg-gray-900 border border-yellow-600/30">
                                    <span className="text-2xl md:text-4xl animate-pulse">🛡️</span>
                                    <span className="text-yellow-500 font-retro text-[6px] md:text-[8px] mt-1 md:mt-2 uppercase tracking-widest">Active</span>
                                </div>
                            ) : (
                                <LazyImage src={item.avatar_url} className="w-full h-full" alt="Artifact" />
                            )}
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/95 p-4 backdrop-blur-2xl">
          <div className="bg-black/80 border-4 border-yellow-600 p-12 rounded-[4rem] text-center max-w-lg shadow-2xl">
            <div className="text-6xl mb-8 animate-float">💰</div>
            <h2 className="text-3xl font-retro text-yellow-500 mb-6 uppercase">INVESTMENT STRATEGY</h2>
            <p className="text-gray-300 font-serif mb-6 text-xl">Ignite Forge for {currentTier.cost} XP?</p>
            <div className="bg-green-900/20 p-4 rounded-2xl border border-green-700/50 mb-8 text-left space-y-2">
                <p className="text-[10px] text-green-400 font-retro uppercase tracking-widest">Profit Calculation</p>
                <ul className="text-xs text-gray-300 space-y-1 list-disc pl-4 font-mono">
                    <li><span className="text-red-400">PAY: {currentTier.cost} XP</span> (Cost)</li>
                    <li><span className="text-blue-400">AUTO-LIST PRICE: {Math.floor(currentTier.cost * 1.3)} XP</span></li>
                    <li><span className="text-yellow-400">YOUR RETURN (IF SOLD): {Math.floor(currentTier.cost * 1.2)} XP</span></li>
                    <li><span className="text-green-400 font-bold">NET PROFIT: +{Math.floor(currentTier.cost * 0.2)} XP</span> (20%)</li>
                </ul>
                <p className="text-[9px] text-gray-500 mt-2 italic">* Buyer inherits item with {currentTier.cost} intrinsic XP.</p>
            </div>
            <div className="flex flex-col gap-4">
              <Button onClick={handleForge} className="bg-green-700 border-green-500 py-6 rounded-3xl hover:bg-green-600">CONFIRM INVESTMENT</Button>
