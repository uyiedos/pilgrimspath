...
const DevotionalView: React.FC<DevotionalViewProps> = ({ onBack, onAddPoints, onUnlockAchievement, userId }) => {
  ...
  // Use local date for logic to avoid timezone issues
  ...
  useEffect(() => {
    const loadContent = async () => {
      setLoading(true);
      try {
        // 1. Check for cached devotional for today (using ISO string for DB consistency)
  ...
  const handleShare = async () => {
      ...
      // Construct the message with excerpts
      ...
      // Fallback to Clipboard if native share failed or wasn't available
      if (!shared) {
          try {
      ...
      // Record Reward (Once per day)
      if (shared && userId) {
          // Use a key that includes the local date string to reset correctly
