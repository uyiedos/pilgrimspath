                            ...
                            <div className="space-y-8">
                                ...
                                <div className="bg-green-900/20 p-4 rounded-xl text-center">
                                    ...
                                    <p className="text-[10px] text-gray-500">
                                        {Math.abs(selectedListing.price - Math.floor(selectedListing.attached_xp * 1.3)) < 5 
        ...
        <div className="relative w-full max-w-7xl">
            ...
            <div className="flex flex-col gap-4 mb-8">
                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                    {(['all', 'game', 'identity', 'sanctuary', 'scripture'] as MarketplaceCategory[]).map(f => (
                        <button
                            key={f}
                ...
                <div className="flex flex-col md:flex-row gap-4 border-t border-white/10 pt-4">
                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                        <span className="text-[9px] font-retro text-gray-500 uppercase flex items-center mr-2 drop-shadow-md whitespace-nowrap">Style:</span>
                        {(['all', 'modern', 'classic', 'voxel'] as StyleFilter[]).map(s => (
                            <button
                                key={s}
                                onClick={() => { AudioSystem.playVoxelTap(); setStyleFilter(s); }}
                                className={`px-4 py-2 rounded-2xl font-bold uppercase text-[8px] md:text-[10px] transition-all border whitespace-nowrap ${styleFilter === s ? 'bg-blue-600 border-blue-400 text-white shadow-md' : 'bg-black/30 backdrop-blur-md border-white/5 text-gray-500 hover:text-white'}`}
                            >
                                {s === 'modern' ? '🧊 Modern' : s === 'classic' ? '👾 Classic' : s === 'voxel' ? '🧱 Voxel' : 'All'}
                            </button>
                        ))}
                    </div>

                    <div className="flex gap-2 overflow-x-auto no-scrollbar md:border-l md:border-white/10 md:pl-4 pb-1">
                        <span className="text-[9px] font-retro text-gray-500 uppercase flex items-center mr-2 drop-shadow-md whitespace-nowrap">Sort:</span>
                        {(['recent', 'price-asc', 'price-desc', 'xp-desc'] as SortOption[]).map(s => (
                            <button
                                key={s}
                                onClick={() => { AudioSystem.playVoxelTap(); setSortBy(s); }}
                                className={`px-4 py-2 rounded-2xl font-bold uppercase text-[8px] md:text-[10px] transition-all border whitespace-nowrap ${sortBy === s ? 'bg-yellow-600 border-yellow-400 text-black shadow-md' : 'bg-black/30 backdrop-blur-md border-white/5 text-gray-500 hover:text-white'}`}
                            >
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-1.5 md:gap-6 pb-20">
                {loading ? (
                    ...
                    filteredListings.map(listing => {
                        ...
                        const isGameAsset = colName.toLowerCase().includes('game');
                        ...
                        <div 
                            ...
                            className={`bg-black/30 backdrop-blur-xl rounded-xl md:rounded-[2.5rem] overflow-hidden border transition-all hover:scale-[1.02] group flex flex-col relative ${isEquipped ? 'border-yellow-600 shadow-[0_0_15px_rgba(202,138,4,0.3)]' : isGameAsset ? 'border-cyan-500/60 hover:border-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.2)]' : 'border-white/5 hover:border-green-500 cursor-pointer hover:shadow-[0_0_20px_rgba(34,197,94,0.2)]'}`}
                        >
                            <div className="aspect-square bg-black relative border-b border-white/5 overflow-hidden">
                                {/* Image Layer - Show even if equipped, but dimmed */}
                                <LazyImage 
                                    src={listing.avatar_history?.avatar_url || ''} 
                                    className={`w-full h-full ${isEquipped ? 'opacity-20 blur-[1px]' : ''}`} 
                                    alt="Artifact Preview" 
                                />

                                {/* Equipped Overlay (The "Badge") */}
                                {isEquipped && (
                                    <div className="absolute inset-0 flex flex-col items-center justify-center z-10">
                                        <span className="text-3xl md:text-5xl animate-pulse drop-shadow-lg">🛡️</span>
                                        <span className="text-yellow-500 font-retro text-[10px] mt-2 uppercase tracking-widest border border-yellow-500/50 px-2 py-1 rounded bg-black/50">Active</span>
                                    </div>
                                )}
                                ...
                                {!isEquipped && (
                                    <div className="absolute top-1 left-1 flex flex-col gap-0.5">
                                        <span className={`text-[5px] md:text-[8px] font-bold px-1 py-0.5 rounded-sm border backdrop-blur-md uppercase shadow-lg ${
                                            isGameAsset ? 'bg-cyan-900/80 border-cyan-400 text-cyan-200' :
                                            styleLabel === 'Modern' ? 'bg-blue-600/80 border-blue-400 text-white' : 
                                            styleLabel === 'Game' ? 'bg-yellow-600/80 border-yellow-400 text-black' : 
                                            'bg-purple-600/80 border-purple-400 text-white'
                                        }`}>
                                            {isGameAsset ? 'GAME' : styleLabel}
                                        </span>
                                    </div>
                                )}
                            ...
                            <div className="p-1.5 md:p-4 flex-1 flex flex-col gap-1 md:gap-2">
                                ...
                                <div className="mt-auto pt-1 md:pt-3 border-t border-white/5 flex flex-col gap-1">
                                    ...
                                    <button 
                                        ...
                                        disabled={processingId === listing.id || !!isEquipped}
