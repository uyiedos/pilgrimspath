...
import { generateGenericResponse } from '../services/geminiService';
...
const GiveawaysView: React.FC<GiveawaysViewProps> = ({ user, onBack, onAddPoints, onSync }) => {
  ...
  const handleAIDraw = async (giveaway: Giveaway) => {
    ...
    try {
        ...
        const message = await generateGenericResponse(`The giveaway "${giveaway.title}" has ended. Winner: ${winnerName}. Write a short 1 sentence blessing.`);

        await supabase.from('chat_messages').insert({
            user_id: user?.id || giveaway.poster_id,
            username: 'SYSTEM_PRIEST',
            message: `✨ RAFFLE RESULT: ${message || "Blessings to the winner."}`,
            type: 'milestone'
        });
