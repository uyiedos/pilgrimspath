-- =================================================================
-- READING PLAN STAGE COMPLETION LOGGING - VERSION 5
-- =================================================================
-- Run this in your Supabase SQL Editor to add proper stage completion tracking
-- This version fixes JSON structure issues and handles missing data gracefully
...
-- 9. CREATE FUNCTION TO GET USER'S STAGE COMPLETION STATUS - FIXED JSON HANDLING
...
BEGIN
    RETURN QUERY
    SELECT 
        -- Generate stage_identifier from reading reference
        regexp_replace(lower(regexp_replace(stage_data->>'reading', '[^a-zA-Z0-9]', '_', 'g')), '_+', '_', 'g') as stage_identifier,
        (stage_data->>'day')::int as stage_number,
        COALESCE(stage_data->>'topic', 'Stage ' || (stage_data->>'day')) as stage_title,
        stage_data->>'reading' as reading_reference,
        ...
        c.user_reflection
    FROM 
        -- Get the plan's days_json and expand it
        (SELECT days_json FROM public.user_plans WHERE id = p_plan_id AND user_id = p_user_id) plan_data,
        jsonb_array_elements(COALESCE(plan_data.days_json, '[]'::jsonb)) AS stage_data
    LEFT JOIN public.plan_stage_completions c ON 
        c.user_id = p_user_id AND 
        c.plan_id = p_plan_id AND 
        c.stage_identifier = regexp_replace(lower(regexp_replace(stage_data->>'reading', '[^a-zA-Z0-9]', '_', 'g')), '_+', '_', 'g')
    WHERE stage_data IS NOT NULL
    ORDER BY (stage_data->>'day')::int;
    
    -- If no stages found, return empty result set
    RETURN;
EXCEPTION WHEN OTHERS THEN
    -- If there's any error with JSON processing, return empty result set
    RETURN;
END;
$$;
